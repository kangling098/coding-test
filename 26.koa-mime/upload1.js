...
let app = new Koa();
let fs = require('fs');
// koa 自己实现中间件 写一个函数返回async函数,内部处理好内容,继续执行即可
...
/**
 * ------WebKitFormBoundaryXGnpgXkrS0PPaiou
 * Content-Disposition: form-data; name="file"; filename="微信图片_20170722154425.jpg"
  ...
  return async(ctx,next)=>{
    await new Promise((resolve,reject)=>{
      ...
      ctx.req.on('end',_=>{
        ...
        //Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryXGnpgXkrS0PPaiou
        if(boundary){ // 提交文件的格式是文件类型 multipart/form-data
          ...
          arr.forEach(line=>{ // 拆分每一行
            ...
            if(head.includes('filename')){
              // 文件
              // content是文件的内容
              let filename = Math.random()+''+new Date(Date.now()).getTime();
